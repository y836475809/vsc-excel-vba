...
suite("Extension E2E Roslyn Test Suite", () => {	
	...
	test("init", async () => {
		...
		const expFileMap = fixtureData.getFileMap();
	...
	test("change content", async () => {
		...
		const expFileMapChanged = fixtureData.getFileMap();
		const orgText = fixtureData.getText("m2.bas");
		fixtureData.update(expFileMapChanged, "m2.bas", `a${orgText}`);
		assert.deepEqual(actFileMapChanged, expFileMapChanged);
